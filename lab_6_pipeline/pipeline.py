...
class CorpusManager:
    ...
    def _scan_dataset(self) -> None:
        ...
        """
        for article_file in self.path_to_raw_txt_data.glob('*_raw.txt'):
            article_id = get_article_id_from_filepath(article_file)
            self._storage[article_id] = from_raw(article_file, Article(None, article_id))
...
class UDPipeAnalyzer(LibraryWrapper):
    ...
    def analyze(self, texts: list[str]) -> list[StanzaDocument | str]:
        ...
        for text in texts:
            ...
            analyzed_texts.append(conllu_text)
