...
from admin_utils.stubs.stanza.models.common.doc import Document
from admin_utils.stubs.stanza.pipeline.core import Pipeline
from core_utils.article.article import get_article_id_from_filepath, Article, ArtifactType
...
class CorpusManager:
    ...
    def _validate_dataset(self) -> None:
        ...
        sorted_raw = sorted(raw_files)
        sorted_meta = sorted(meta_files)

        for ind, (raw, meta) in enumerate(zip(sorted_raw, sorted_meta), 1):
            if (ind != get_article_id_from_filepath(raw) or
                    ind != get_article_id_from_filepath(meta) or not
                    (raw.stat().st_size and meta.stat().st_size)):
                raise InconsistentDatasetError
...
class TextProcessingPipeline(PipelineProtocol):
    ...
    def run(self) -> None:
        ...
        """
        if self._analyzer:
            files = self._analyzer.analyze([article.text for article in
                                            self._corpus.get_articles().values()])
            for ind, article in self._corpus.get_articles().items():
                to_cleaned(article)
                article.set_conllu_info(files[ind - 1])
...
class StanzaAnalyzer(LibraryWrapper):
    ...
    def from_conllu(self, article: Article) -> CoNLLUDocument:
        ...
        """
        return CoNLL.conll2doc(input_file=article.get_file_path(
...
class POSFrequencyPipeline:
    ...
    def run(self) -> None:
        ...
        """
        for ind, article in self._corpus.get_articles().items():
            if not article.get_file_path(
                    kind=ArtifactType.STANZA_CONLLU).stat().st_size:
